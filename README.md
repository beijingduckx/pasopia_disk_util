...
## 構成
- `DiskUtil.wav`  
  T-BASIC, T-BASIC7 CLOAD用の音声データ (`DSKUTL.bas`のプログラムが入っています)
- `DiskImageCopy.ps1`  
 ディスクイメージ書き込み用PowerShellプログラム (Windows用)
...
1. 画面の表示に従って操作します。メニューからは  
   ...
   3: 終了  
   を選択します
1. `Do BASIC logical format` で `y`とすると、T-BASIC7 用の論理フォーマットを行います  
(T-DISKBASIC用の論理フォーマットにもなっていると思いますが、手元になく未確認です)
...
> - PASOPIAより先にディスクドライブの電源を入れてください (FDCの初期化は、このプログラムでは行っていないためです)


## 制限
- 5インチ2D専用です
- イメージ書き込み・コピーは、トラック0 サイド0 を処理をしません (128bytes/sector部を避けています)  
   フォーマットは、トラック0を128bytes/sectorで初期化します
